...
public sealed partial class PinnedConfigurationTests
{
    ...
    public void CiWorkflowPinsCrossPlatformGates()
    {
        ...
        Assert.Contains("Restore pinned ripgrep oracle archive", releaseGateWorkflow, StringComparison.Ordinal);
        Assert.Contains("eng/restore-ripgrep-oracle.sh", releaseGateWorkflow, StringComparison.Ordinal);
        ...
        Assert.Contains("full pinned macOS tests", releaseGateWorkflow, StringComparison.Ordinal);
        Assert.Contains("full pinned Windows tests", releaseGateWorkflow, StringComparison.Ordinal);
        Assert.DoesNotContain("full pinned tests (${{ matrix.rid }})", releaseGateWorkflow, StringComparison.Ordinal);
        ...
        Assert.Contains("release native macOS gate", releaseGateWorkflow, StringComparison.Ordinal);
        Assert.Contains("release native Windows gate", releaseGateWorkflow, StringComparison.Ordinal);
        Assert.DoesNotContain("release native macOS gate (${{ matrix.rid }})", releaseGateWorkflow, StringComparison.Ordinal);
        Assert.DoesNotContain("release native Windows gate (${{ matrix.rid }})", releaseGateWorkflow, StringComparison.Ordinal);
    ...
    public void HostedReleaseGatesProvisionPinnedRipgrepOracle()
    {
        ...
        Assert.Contains("eng/restore-ripgrep-oracle.ps1", workflow, StringComparison.Ordinal);
        Assert.DoesNotContain("Build pinned ripgrep oracle", workflow, StringComparison.Ordinal);
        Assert.DoesNotContain("eng/setup-ripgrep-oracle.sh", workflow, StringComparison.Ordinal);
        Assert.DoesNotContain("eng/setup-ripgrep-oracle.ps1", workflow, StringComparison.Ordinal);
        ...
        Assert.Contains("archive_path", script, StringComparison.Ordinal);
        Assert.Contains("archive_sha256", script, StringComparison.Ordinal);
        Assert.Contains("ripgrep_oracle", script, StringComparison.Ordinal);
        Assert.Contains("unzip -q -o \"$ARCHIVE_PATH\" -d \"$ROOT\"", script, StringComparison.Ordinal);
        ...
        Assert.Contains("archive_sha256 = \"%s\"", captureScript, StringComparison.Ordinal);
        ...
        Assert.Contains("archive_path", windowsScript, StringComparison.Ordinal);
        Assert.Contains("archive_sha256", windowsScript, StringComparison.Ordinal);
        Assert.Contains("Expand-Archive -LiteralPath $ArchivePath -DestinationPath $Root -Force", windowsScript, StringComparison.Ordinal);
    ...
    public void HostedRipgrepOracleRowsCoverReleaseTestRids()
    {
        ...
        {
            (string rid, string archivePath, string archiveSha256, string path, string sha256, string pcre2Path, string pcre2Sha256) = rows[index];
    ...
    public void HostedOracleCaptureUsesGitHubHostedUnixRunners()
    {
        ...
        Assert.Contains("tests/oracles/ripgrep/linux-x64.zip", workflow, StringComparison.Ordinal);
        Assert.Contains("tests/oracles/ripgrep/linux-x64.lock", workflow, StringComparison.Ordinal);
        Assert.Contains("tests/oracles/ripgrep/${{ matrix.rid }}.zip", workflow, StringComparison.Ordinal);
        Assert.Contains("tests/oracles/ripgrep/${{ matrix.rid }}.lock", workflow, StringComparison.Ordinal);
    ...
    public void MsBuildWarningGateEvaluatesImportedProperties()
    {
        ...
        Assert.Contains("-getProperty:IsAotCompatible", script, StringComparison.Ordinal);
        Assert.Contains("-getProperty:EnableTrimAnalyzer", script, StringComparison.Ordinal);
        Assert.Contains("-getProperty:EnableAotAnalyzer", script, StringComparison.Ordinal);
        Assert.Contains("-getProperty:TrimMode", script, StringComparison.Ordinal);
        Assert.Contains("-getProperty:ILLinkTreatWarningsAsErrors", script, StringComparison.Ordinal);
        Assert.Contains("-getProperty:TrimmerSingleWarn", script, StringComparison.Ordinal);
        Assert.Contains("-getProperty:SuppressTrimAnalysisWarnings", script, StringComparison.Ordinal);
        Assert.Contains("-getProperty:RuntimeFrameworkVersion", script, StringComparison.Ordinal);
        Assert.Contains("-getProperty:PublishAot", script, StringComparison.Ordinal);
        Assert.Contains("-getProperty:NativeLib", script, StringComparison.Ordinal);
        Assert.Contains("-getProperty:OutputType", script, StringComparison.Ordinal);
        ...
        Assert.Contains("is_runtime_aot_project", script, StringComparison.Ordinal);
        Assert.Contains("check_equals \"$relative_project\" \"TrimMode\"", script, StringComparison.Ordinal);
        Assert.Contains("check_true \"$relative_project\" \"ILLinkTreatWarningsAsErrors\"", script, StringComparison.Ordinal);
        Assert.Contains("check_false \"$relative_project\" \"TrimmerSingleWarn\"", script, StringComparison.Ordinal);
        Assert.Contains("check_false \"$relative_project\" \"SuppressTrimAnalysisWarnings\"", script, StringComparison.Ordinal);
        Assert.Contains("check_equals \"$relative_project\" \"RuntimeFrameworkVersion\"", script, StringComparison.Ordinal);
        Assert.Contains("check_true \"$relative_project\" \"IsAotCompatible\"", script, StringComparison.Ordinal);
        Assert.Contains("check_true \"$relative_project\" \"EnableTrimAnalyzer\"", script, StringComparison.Ordinal);
        Assert.Contains("check_true \"$relative_project\" \"EnableAotAnalyzer\"", script, StringComparison.Ordinal);
        Assert.Contains("check_true \"$relative_project\" \"PublishAot\"", script, StringComparison.Ordinal);
        Assert.Contains("check_equals \"$relative_project\" \"NativeLib\"", script, StringComparison.Ordinal);
        Assert.Contains("check_equals \"$relative_project\" \"OutputType\"", script, StringComparison.Ordinal);
    ...
    public void VendoredUnicodeTablesMatchPinnedRegexSyntaxVersion()
    {
        ...
        Assert.Contains("private static ReadOnlySpan<int> PerlSpace", regexUnicodeTables, StringComparison.Ordinal);
    ...
    public void HyperfineBenchSuiteCoversReleaseGateWorkloads()
    {
        ...
        Assert.Contains("Peak RSS allows the measured Native AOT fixed RSS floor", script, StringComparison.Ordinal);
        ...
        Assert.Contains("run_hyperfine_interleaved", script, StringComparison.Ordinal);
        Assert.Contains("bench/hyperfine_interleaved.py", script, StringComparison.Ordinal);
        Assert.DoesNotContain("check_time_gate_combined", script, StringComparison.Ordinal);
        ...
        Assert.Contains("GATE_LARGE_BOUNDED_UNICODE_CLASS_RUNS=\"5\"", script, StringComparison.Ordinal);
        Assert.Contains("GATE_LARGE_BOUNDED_UNICODE_CLASS_WARMUP=\"5\"", script, StringComparison.Ordinal);
        ...
        Assert.Contains("interleaved ABBA performance gate", script, StringComparison.Ordinal);
        ...
        Assert.Contains("retrying failed gate workload (", script, StringComparison.Ordinal);
        Assert.Contains("exceeded the performance gate after $GATE_RETRY_FAILED_WORKLOADS retries", script, StringComparison.Ordinal);
        Assert.Contains("exceeded the peak RSS gate", script, StringComparison.Ordinal);
        Assert.Contains("paired ABBA", readme, StringComparison.Ordinal);
        Assert.Contains("geometric mean", readme, StringComparison.Ordinal);
        Assert.Contains("median round ratio", readme, StringComparison.Ordinal);
        Assert.Contains("rg`, Scout, Scout, then `rg", readme, StringComparison.Ordinal);
        Assert.Contains("ten timing samples", readme, StringComparison.Ordinal);
        ...
        Assert.Contains("Median peak RSS", readme, StringComparison.Ordinal);
        Assert.Contains("leading rg/Scout", readme, StringComparison.Ordinal);
        Assert.Contains("pair; the trailing rg", readme, StringComparison.Ordinal);
        Assert.Contains("child peak RSS is cumulative", readme, StringComparison.Ordinal);
        Assert.Contains("filesystem-cache or hosted", design, StringComparison.Ordinal);
        Assert.Contains("_ABBA_ROLES = (\"rg\", \"scout\", \"scout\", \"rg\")", interleaved, StringComparison.Ordinal);
        Assert.Contains("position <= 2", interleaved, StringComparison.Ordinal);
        ...
        Assert.Contains("test_abba_resists_monotonic_drift_without_masking_regression", interleavedTests, StringComparison.Ordinal);
        Assert.Contains("1.2053249575958502", interleavedTests, StringComparison.Ordinal);
        ...
        Assert.Contains("use five measured ABBA rounds and five", readme, StringComparison.Ordinal);
        Assert.Contains("warmup ABBA rounds by default", readme, StringComparison.Ordinal);
        Assert.Contains("ten measured samples and ten", readme, StringComparison.Ordinal);
        Assert.Contains("override the number of rounds", readme, StringComparison.Ordinal);
