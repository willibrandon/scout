...
def emit_pair_table(name: str, pairs: list[tuple[int, int]]) -> None:
    print(f"    private static readonly UnicodeScalarPair[] {name} =")
    print("    [")
    for first, second in pairs:
        print(f"        new(0x{first:06X}, 0x{second:06X}),")
    print("    ];")
    print()
...
def main() -> None:
    ...
    print("    }")
    print()
    print("    private static bool Contains(ReadOnlySpan<UnicodeScalarPair> pairs, int first, int second)")
    print("    {")
    print("        int low = 0;")
    print("        int high = pairs.Length - 1;")
    print("        while (low <= high)")
    print("        {")
    print("            int middle = low + ((high - low) / 2);")
    print("            UnicodeScalarPair pair = pairs[middle];")
    print("            if (first < pair.First || first == pair.First && second < pair.Second)")
    print("            {")
    print("                high = middle - 1;")
    print("                continue;")
    print("            }")
