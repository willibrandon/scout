...
|---|---|
| `Version` / `VersionPrefix` | `0.3.0` (Scout's own SemVer, decoupled from ripgrep 15.1.0) |
| `AssemblyVersion` / `FileVersion` | derived from `Version` |
| `AssemblyInformationalVersion` | `0.3.0+ripgrep.15.1.0.4857d6fa67` (SemVer build metadata carries the upstream coordinate) |
| `Product` | `Scout` |
...
```
scout 0.3.0 (ripgrep 15.1.0 compatible, rev 4857d6fa67)
```
