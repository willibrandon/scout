...
### 1.1 Rename cost
The name appears in namespace identifiers, the assembly/binary name, package IDs, help/man/completion artifacts, the version banner, top-level stderr program prefixes, `--debug`/`--trace` structural fields, homepage/issue URLs, author-credit placement, config environment variable documentation, and default ignore-file names. Rename work is therefore scripted and gated, not a naive string replacement. The release identity contract is `docs/IDENTITY-REBRAND.md`: behavior remains pinned to ripgrep, Scout-owned identity surfaces say Scout, and upstream provenance/legal material remains ripgrep.
...
- **Standard-input EOF.** Windows pipe readers translate `ERROR_BROKEN_PIPE` and `ERROR_NO_DATA` from `ReadFile` into a zero-byte read, matching stream EOF after the writer closes. Downstream stdout/stderr pipe closure remains a write-side condition that exits successfully.
...
>
> The complete release matrix is reproduced in GitHub-hosted CI: Unix jobs run `spike/build-unix.sh` for `osx-arm64`, `osx-x64`, `linux-x64`, and `linux-arm64`; Windows jobs run `spike/build-windows.ps1` for `win-x64` and `win-arm64`. A green CI run for the commit under test is the authoritative M0 proof for the four non-local OS/architecture environments. *(Correction of record: earlier revisions of this note first wrongly said no SDK was installed, then prematurely claimed a pass; both were wrong. This reflects actually-executed local transcripts plus the CI proof mechanism.)*
...
The CLI creates one operation-scoped `RegexEnginePlan` from the complete ordered pattern set. It selects and owns either a native `RegexSearchPlan` or a `Pcre2SearchPlan`, so default and automatic engine selection compile each candidate at most once before search dispatch. The native plan combines and parses the patterns once, derives record scope, line-terminator rules, literal acceleration, conservative prefilters, and capture mappings from that syntax tree, and owns the authoritative matcher. Each input buffer produces one ordered match stream whose spans are replayed by standard, context, JSON, vimgrep, statistics, and replacement output. An HIR-style finite-language extractor proves exact literal sets with stable alternation unions, concatenation products, and bounded repetition products. It preserves preference order and applies the pinned regex-syntax limits for class expansion, repetition, literal length, and total language size; unsupported or over-limit syntax uses the general automaton. Captured exact alternatives retain ordered pattern identity through a parsed `PatternSet`.
...
Authoritative character classes are lowered once per parsed atom and effective semantic option scope. Bounded repetitions emit each Thompson state while sharing the canonical scalar-range payload and direction-specific UTF-8 compilation plan; construction budgets charge each shared payload once.
...
- **Context/stats buffer boundaries** follow the resolved thread count. Parallel standard output treats statistics as a final output buffer and writes the configured context separator before it when a context body was emitted. Serial output writes the statistics directly after the required blank line, including one-file, `-j1`, and sorted searches.
...
**Config environment identity.** Scout reads `SCOUT_CONFIG_PATH` first and treats it as primary. If `SCOUT_CONFIG_PATH` is set and non-empty, Scout uses it exclusively; a read error is reported against `SCOUT_CONFIG_PATH` and does not fall back to `RIPGREP_CONFIG_PATH`. Only when `SCOUT_CONFIG_PATH` is unset or empty does Scout consult `RIPGREP_CONFIG_PATH` as a compatibility fallback. `--no-config` disables both. The differential harness clears/controls both variables so behavior comparisons are isolated.

#### 4.10.2 Identity vs parity vs provenance
Scout's release contract separates three surfaces:

- **Behavioral parity:** search stdout bytes, exit codes, error message bodies, debug message bodies, file-type data, globs, fixtures, and other behavior remain byte-identical to the pinned `rg` after narrow identity-field normalization where needed.
- **Scout identity:** version banners, top-level stderr program prefixes, help/man/completion command identity, homepage/issue URLs, `--debug`/`--trace` program/category/source-location fields, `SCOUT_CONFIG_PATH`, `.scoutignore`, and Scout-owned product voice use Scout naming and are pinned by Scout golden tests plus the rebrand audit.
- **Provenance/legal:** vendored upstream data, oracle tooling, `UPSTREAM.md`, source provenance comments, third-party notices, pinned commit names, and license attribution retain ripgrep/upstream identifiers.

This is the only accepted runtime carve-out from byte-identical output: no accepted **behavioral** deviations are permitted at release, while identity surfaces are intentionally Scout-specific and enumerated in `docs/PARITY.md`.
...
- **Config env vars** = `SCOUT_CONFIG_PATH` primary with `RIPGREP_CONFIG_PATH` compatibility fallback (§4.10.1).
...
**`spike/build-unix.sh`** (Unix driver for `osx-arm64`, `osx-x64`, `linux-x64`, and `linux-arm64`; the runtime-archive set is read from the `microsoft.netcore.app.runtime.nativeaot.<RID>` pack — note the bootstrapper is the object file `libbootstrapperdll.o`, and `libaotminipal.a` is required)
```sh
#!/usr/bin/env sh
set -eu
