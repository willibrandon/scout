...
- **Native entry** = AOT static library + platform C entry driver (runtime self-initializes on first managed call — §4.1.1). **Executed and PASSED on osx-arm64 and osx-x64** (0xFF argv byte round-tripped; Appendix A transcript). Reproducing on the remaining four RIDs is the M0 CI gate before the byte-boundary blocker is fully closed.
